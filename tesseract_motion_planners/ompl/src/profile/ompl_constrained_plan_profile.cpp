/**
 * @file ompl_constrained_plan_profile.cpp
 * @brief
 *
...
{
OMPLConstrainedPlanProfile::OMPLConstrainedPlanProfile(const tinyxml2::XMLElement& xml_element)
{
...
void OMPLConstrainedPlanProfile::setup(OMPLProblem& prob) const
{
  ...
  auto css = std::make_shared<ompl::base::ProjectedStateSpace>(state_space_ptr, constraint);

  auto csi = std::make_shared<ompl::base::ConstrainedSpaceInformation>(css);

  // Create Simple Setup from state space
  prob.simple_setup = std::make_shared<ompl::geometric::SimpleSetup>(csi);
                                                  ...
                                                  int /*index*/) const
{
  ...
  if (prob.state_space == OMPLProblemStateSpace::REAL_STATE_SPACE)
  {
    ...
    for (std::size_t i = 0; i < joint_solutions.size(); ++i)
    {
      ...
      {
        CONSOLE_BRIDGE_logDebug("In OMPLConstrainedPlanProfile: Start state has invalid bounds");
      }
                                                  ...
                                                  int /*index*/) const
{
  ...
  if (prob.state_space == OMPLProblemStateSpace::REAL_STATE_SPACE)
  {
    ...
    {
      CONSOLE_BRIDGE_logDebug("In OMPLConstrainedPlanProfile: Start state is outside limits");
    }
...
tinyxml2::XMLElement* OMPLConstrainedPlanProfile::toXML(tinyxml2::XMLDocument& doc) const
{
...
ompl::base::StateValidityCheckerPtr OMPLConstrainedPlanProfile::processStateValidator(OMPLProblem& prob) const
{
...
void OMPLConstrainedPlanProfile::processMotionValidator(
    OMPLProblem& prob,
...
void OMPLConstrainedPlanProfile::processOptimizationObjective(OMPLProblem& prob) const
{
